...
export const actions: Actions = {
    login: async ({ cookies, request }) => {
        ...
        const db_pass = await db.query("SELECT password FROM user_info WHERE email = ?", [email]);
        const hash_pass: string = createHash('sha512').update(password).digest('hex');
        if (db_pass[0].toString() !== hash_pass) return fail(400, {email, invalid_login: true});

        // create JWT token
        const user = await db.query("SELECT email, team_name, first_name, last_name FROM user_info WHERE email = ?", [email]);
        const token: string = jwt.sign({user}, env.JWT_SECRET, {expiresIn: '1h'});
